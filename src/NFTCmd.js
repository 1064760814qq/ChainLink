...
const mintNft = new Command("mint-nft")
      ...
      .option("--privateKey <value>","Private key to use",constants[key].NFT_PRIVATE_KEY)
...
const startNewBlindBox = new Command("set-new-blind-box")
      ...
      .action(async function (args) {
        ...
        const tx1 = await nftInstance.startNewBlindBox(args.total,args.date,args.baseuri);
        await waitForTx(args.provider, tx1.hash);
...
const exchangeNft = new Command("exchange")
      ...
      .action(async function (args) {
        ...
        const tx1 = await nftInstance[
          "exchange(uint256,address,uint128,uint128)"
        ](args.id, args.to, args.life, args.grade);
        await waitForTx(args.provider, tx1.hash);
